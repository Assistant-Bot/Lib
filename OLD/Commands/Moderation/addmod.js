exports.setup = {
    'name' : 'addmod',
    'aliases' : ['mod'],
    'usage' : '`addmod user @user` - Add a user to your mods list.\n`addmod role roleid/search` - Add a role to the mods list.',
    'details' : 'Add a mod role or user to your server',
    'examples' : [''],
    'permission' : 2,
...
try {
  ...
  try {
    ...
    if(client.servers.get(message.guild.id)[type].includes(target)) return message.channel.send(`${assistant.emoji.redtick} Already a mod.`);
    ...
    message.channel.send(`${assistant.emoji.check} **${getName(target, type)}** has been added to the modlist!`);
  } catch (err) {
