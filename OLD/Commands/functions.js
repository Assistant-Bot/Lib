...
exports.database = {
  ...
  remove: async function(place, data) {
    ...
    if(!dr) return false;
    if(!dr[data.child]) return `Child: ${data.child} not found in ${data.parent}`;
...
exports.emoji = {
 ...
 exports.runscripts = async function (scrpts, trgt, ASSISTANTDEBUGCLIENT) {
  ...
  const assistant = {
    emojis : require('./functions.js').emoji
  }
  ...
  function findUser(name) {
    return ASSISTANTDEBUGCLIENT.resolveMember(trgt.guild.id, name);
  }
  //CODE
  let msg = trgt;
