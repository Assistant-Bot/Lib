...
if(usr.roles.has(findRole('Voice Ban')[0].id)) {
  em.setColor('#ff0000');
  assistant.vars.msg2 = "This user is already Voice banned."
  return;
} else {
  usr.addRole(findRole('Voice Ban')[0].id);
  assistant.vars.msg2 = `**${msg.member.user.username}** Voice Banned \`${usr.user.username}\`\nReason: ${r}`;
  em.setColor('#ff0000')
  em.addField('Voice Ban', `**Moderator:** ${msg.member}\n**Reason:** ${r}`);
