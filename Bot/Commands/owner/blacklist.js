    ...
    async onRun(bot, msg, args, Util, emojis) {
        ...
        try {
            ...
            if (args[0] == 'view') {
                ...
                return msg.channel.send(emojis.greentick + ' Successfully blacklisted `' + wrapped[1] + '` with reason: `' + reason + '`');
            } else if (args[0] == 'remove') {
                if (!wrapped[1]) return Util.sendError(msg, emojis, 'custom', 'Invalid user.');
                let reason = (!wrapped[2]) ? "No reason provided" : wrapped[2];
                Util.blacklist(wrapped[1], 1, msg.author.id, reason);
                return msg.channel.send(emojis.greentick + ' Successfully unblacklisted `' + wrapped[1] + '` with reason: `' + reason + '`');
